...
class GantryUnlockActionNode : public plansys2::ActionExecutorClient {
public:
  GantryUnlockActionNode()
    : plansys2::ActionExecutorClient("unlock", 100ms) {
    client_ = this->create_client<emulation_msgs::srv::TriggerGantry>("/gantry_emulator_service");
    RCLCPP_INFO(this->get_logger(), "GantryUnlockActionNode constructed");
    RCLCPP_INFO(this->get_logger(), "GantryEmulatorAction registered for 'unlock'");
  }
protected:
  void do_work() override {
    RCLCPP_INFO(this->get_logger(), "GantryUnlockActionNode::do_work() called");
...
int main(int argc, char ** argv)
{
  ...
  auto action = std::make_shared<GantryUnlockActionNode>();
  action->set_parameter(rclcpp::Parameter("action_name", "unlock"));
