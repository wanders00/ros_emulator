...
class RobotMountActionNode : public plansys2::ActionExecutorClient {
public:
  RobotMountActionNode()
    : plansys2::ActionExecutorClient("mount", 100ms) {
    client_ = this->create_client<emulation_msgs::srv::TriggerRobot>("/robot_emulator_service");
    RCLCPP_INFO(this->get_logger(), "RobotMountActionNode constructed");
    RCLCPP_INFO(this->get_logger(), "RobotEmulatorAction registered for 'mount'");
  }
protected:
  void do_work() override {
    RCLCPP_INFO(this->get_logger(), "RobotMountActionNode::do_work() called");
...
int main(int argc, char ** argv)
{
  ...
  auto action = std::make_shared<RobotMountActionNode>();
  action->set_parameter(rclcpp::Parameter("action_name", "mount"));
