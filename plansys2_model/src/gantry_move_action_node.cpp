...
class GantryMoveActionNode : public plansys2::ActionExecutorClient {
public:
  GantryMoveActionNode()
    : plansys2::ActionExecutorClient("move_gantry", 100ms) {
    client_ = this->create_client<emulation_msgs::srv::TriggerGantry>("/gantry_emulator_service");
    RCLCPP_INFO(this->get_logger(), "GantryMoveActionNode constructed");
    RCLCPP_INFO(this->get_logger(), "GantryEmulatorAction registered for 'move_gantry'");
  }
protected:
  void do_work() override {
    RCLCPP_INFO(this->get_logger(), "GantryMoveActionNode::do_work() called");
...
int main(int argc, char ** argv)
{
  ...
  auto action = std::make_shared<GantryMoveActionNode>();
  action->set_parameter(rclcpp::Parameter("action_name", "move_gantry"));
