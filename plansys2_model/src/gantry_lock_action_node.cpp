...
class GantryLockActionNode : public plansys2::ActionExecutorClient {
public:
  GantryLockActionNode()
    : plansys2::ActionExecutorClient("lock", 100ms) {
    client_ = this->create_client<emulation_msgs::srv::TriggerGantry>("/gantry_emulator_service");
    RCLCPP_INFO(this->get_logger(), "GantryLockActionNode constructed");
    RCLCPP_INFO(this->get_logger(), "GantryEmulatorAction registered for 'lock'");
  }
protected:
  void do_work() override {
    RCLCPP_INFO(this->get_logger(), "GantryLockActionNode::do_work() called");
...
int main(int argc, char ** argv)
{
  ...
  auto action = std::make_shared<GantryLockActionNode>();
  action->set_parameter(rclcpp::Parameter("action_name", "lock"));
